...
//	然后第四个SampleTime，就是指定通道的采样时间
	...
	DMA_InitStruct.DMA_Mode = DMA_Mode_Circular; // DMA传输模式:循环模式
	...
	// 由于现在使用的是硬件触发，第二个条件:触发源有触发信号不满足，因为这里是硬件出发，ADC还没启动，不会有触发信号，所以这里DMA使能后不会立刻工作，它只会在工位上等待
	...
	// 7.ADC软件触发(下达开工指令)
	// 配置循环模式之后，把ADC触发，直接放在初始化的最后一行
	// 当ADC触发之后，ADC连续转换，DMA循环转运，两者一直在工作，始终把最新的转换结果，刷新到SRAM数组里，当我们想要数据的时候，随时去数组里取就行了，这样最后面的AD_GetValue函数就完全不需要了
	ADC_SoftwareStartConvCmd(ADC1, ENABLE);
	
	// 在 ADC 连续扫描 + DMA 循环模式 下，不需要判断和清除标志位
