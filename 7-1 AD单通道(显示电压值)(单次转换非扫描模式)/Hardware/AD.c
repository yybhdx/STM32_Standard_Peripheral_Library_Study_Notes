...
void AD_Init(void)
{
	...
	// 为了满足ADC时钟不超过14MHz的推荐值，一个常见的配置是选择6分频
	...
	ADC_InitStruct.ADC_ContinuousConvMode = DISABLE; // 关闭连续转换模式 // 要使用ADC的ADC_ContinuousConvMode（连续转换模式），那么这个参数就要改成ENABLE
	...
	// 返回复位校准寄存器的当前状态
	// 等待复位完成：循环检查 ADC_GetResetCalibrationStatus(ADCx)的返回值，直到它变为 RESET(0)。
	...
	// 查询指定ADC（模数转换器）的校准状态
	// 等待校准完成：循环检查 ADC_GetCalibrationStatus(ADCx)的返回值，直到它变为 RESET
...
uint16_t AD_GetValue(void)
{
	...
	// 参数 ADC1:指定要读取数据的ADC外设为ADC1
	// 返回值：返回一个16位的无符号整数，代表ADC的转换结果值。
