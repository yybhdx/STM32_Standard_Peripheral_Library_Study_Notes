...
void pwm_init(void)
{
	...
	TIM_OCInitStruct.TIM_Pulse = 0; // 脉冲值(CCR)取值范围为500 - 2500 对应的就是0.5ms ~ 2.5ms (因为每1us计数一次,ARR值为20000，它们相当于ARR的大小就是相对于20ms(周期)的大小)
...
{
	// TIM_SetCompare2对应通道2
	// TIM_SetCompare2:设置定时器通道 2 的捕获/比较寄存器 CCR1 的值(运行时可以用来更改占空比)
	// TIM2:要操作的定时器
	// Compare:要写入 CCR1 的值
