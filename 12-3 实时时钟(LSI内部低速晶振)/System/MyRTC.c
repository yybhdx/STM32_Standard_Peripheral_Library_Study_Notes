...
//	RCC_LSEConfig(RCC_LSE_ON);
	...
	// 2.启动RTC的时钟(计划使用LSI),并等待LSI时钟启动完成
	// RCC_LSICmd()使能或失能内部低速时钟（LSI）
	// ENABLE使能内部低速时钟
	RCC_LSICmd(ENABLE);
...
//	while(RCC_GetFlagStatus(RCC_FLAG_LSERDY) == RESET); //如果返回标志位状态为RESET，则未置为完成，继续等待
	
	// 等待LSI时钟启动完成
	// RCC_GetFlagStatus() 检查复位与时钟控制单元（RCC）中各种状态标志位的当前状态
	// RCC_FLAG_LSIRDY：内部低速时钟（LSI）就绪。
	while(RCC_GetFlagStatus(RCC_FLAG_LSIRDY) == RESET); //如果返回标志位状态为RESET，则未置为完成，继续等待
...
//	RCC_RTCCLKConfig(RCC_RTCCLKSource_LSE);
	...
	// RCC_RTCCLKSource_LSI：选择内部低速 RC 振荡器 (LSI，频率约为 40kHz) 作为 RTC 时钟。LSI 的成本较低，但精度相对较差，适合对时钟精度要求不高的场合 。
...
//	// LSE的频率为32.768KHZ 进行32768分频，就是1HZ了。所以分频系数我们给32768-1
//	RTC_SetPrescaler(32768-1);

	// 因为LSI的频率为40KHZ 进行40000分频，就是1HZ了。所以分频系数我们给40000-1
