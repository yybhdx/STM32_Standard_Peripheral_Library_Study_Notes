...
// 定义DMA转运的源端数组
const uint8_t DataA[] = {0X01, 0X02, 0X03, 0X04}; // 源端数组
...
int main(void)
{
	...
	// 显示地址
	OLED_ShowHexNum(1, 8, (uint32_t)DataA, 8); // 0800 11B8(现在可能是模块里的那个Size变量被分配到了0的位置，所以这个地址就往后偏移了2个字节)
	OLED_ShowHexNum(3, 8, (uint32_t)DataB, 8); // 2000 0002
	...
	while (1)
	{
		// 给DataA加一个const，将DataA定义在Flash里面，那么下面这里DataA[]++就不能要了 因为是常变量
//		// 变化一下源端A的数据，给它们自增1
//		DataA[0]++;
//		DataA[1]++;
//		DataA[2]++;
