...
/**
  ...
  * 返 回 值：无
  */
/*PWM驱动直流电机 PWM信号输入端接在了PA2(对应TIM2_CH3) 所以GPIO要改成PIN_2
TIM_OC1Init()要改成TIM_OC3Init(),因为用的是通道3
TIM_SetCompare1()要改成TIM_SetCompare3(),因为用的是通道3*/
...
void PWM_Init(void)
{
	...
	// PWM输出对应的PA的GPIOA挂载在APB2外设总线哈桑，开启GPIOA的时钟
